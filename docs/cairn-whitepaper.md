...
That run is capped, at 16 474 headers, about 3.0 MB, and the cap is a
...
weighed from about eleven days after the loss until months or years after
...
loss at 526 494 headers and 96 MB, thirty two times the cap, and nothing
...
fifty odd halvings against the fourteen the count is set for, and it took a
...
against every forger up to 43% of the world's work over a thirty year
...
keeping three points for the difference between a staircase of halvings
...
blocks from the tip and the measured guarantee is about 1 240, while
`MAX_REORG_DEPTH` is 1 024 and the effective undo limit is the
lesser of that and the network's burial. So a newcomer put on a branch at
the far end of the guarantee cannot be carried back onto the real chain by
the ordinary reorganisation rule: the switch it would need is deeper than
the deepest one the node will make. An earlier version of this paragraph
said the opposite, that the depth was shallower than the deepest
reorganisation, which its own two figures refute.

This is a gap between a probabilistic bound and a retention policy, not a
break in either. Closing it means one of three things: resolving the draw
finer so the guarantee falls under the limit, raising the limit above the
guarantee and paying for the undo data that costs, or accepting that a
newcomer this unlucky has to be started again and saying so where an
operator reads it. It is stated here rather than answered because the
answer changes a rule, and a rule should not change in a paragraph.
...
hold to 40% of the world's work and a depth of about 1 240 blocks, and
...
16 474, and a chain whose difficulty at the tip has fallen far below its
...
way in from about eleven days after the loss until months or years after
