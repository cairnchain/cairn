...
cheap: every header a weighing opens names this network, every one is
dated after the network opened, and the chain is shown to start from the
block the software pins, opened at position zero of the tip's own
commitment. A chain that starts anywhere else is refused whatever work
stands behind it. The audit that closed the last of those reads the
other way round: until it was closed, a chain started from another first
block, carrying the right network number and dated after the opening,
was weighed on its work alone and taken.
...
burial, so on the public test network, whose burial is the full 1 024, a
newcomer put on a branch at the far end of the guarantee is somewhere the
ordinary reorganisation rule can carry it back from. Devnet is the
exception and is meant to be: its burial is 32 so a throwaway chain
matures in minutes, it undoes 32 blocks against a guarantee of 633, and
what makes that acceptable is that it names no seed, so a newcomer there
joins its own operator's machines and no stranger is placed to weigh it
onto a branch. This paragraph named the lesser-of rule and then concluded
as though the lesser were always the constant. It was
...
The implementation is roughly 59 000 lines of Rust with 1 800 tests, no
